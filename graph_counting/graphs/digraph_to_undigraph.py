...
def transform_digraph_to_undrigraph(graph_in, graph_out):
	...
	for line in graph_in:
		...
		if start_vertex not in edges:
			edges[start_vertex] = []
        if end_vertex in set(edges[start_vertex]):
		print("there is a repeated edge: ", line)
		edges[start_vertex].append(end_vertex)
		if end_vertex not in edges:
			edges[end_vertex] = []		
		edges[end_vertex].append(start_vertex)	
		
	for item in sorted(edges):
		for another_vertex in sorted(set(edges[item])):
			graph_out.write(str(item) + ' ' + str(another_vertex) + '\n')
