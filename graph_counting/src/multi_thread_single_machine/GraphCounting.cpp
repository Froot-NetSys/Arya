...
#define factor_4_chain 2
#define factor_4_cycle 8
#define factor_4_motif_4 1
#define factor_4_motif_5 2
#define factor_4_clique 24
...
void pattern_counting(Graph &large_graph, Pattern &graph_pattern, uint64_t sampling_times) {
    ...
    default_random_engine this_rand_generator;
    this_rand_generator.seed(std::chrono::system_clock::now().time_since_epoch().count());
    tie(this_prob_inverse, this_sampled_times) = estimate_pattern(large_graph, graph_pattern, sampling_times, lost_edge_cases, this_rand_generator);
...
void multi_thread_pattern_counting(Graph &large_graph, Pattern &graph_pattern, uint64_t sampling_times,
    ...
    for (uint32_t i = 0; i <= graph_pattern.other_edges_.size(); i++)
    {
        cout << i << " " << lost_edge_cases[i] << endl;
    }
}
