...
*/
SamplingTreeNode *odd_cycle_sampler(Graph &graph, uint32_t cycle_length, default_random_engine &this_rand_generator) {
...
*/
SamplingTreeNode *star_sampler(Graph &graph, uint32_t petal_num, default_random_engine &this_rand_generator) {
    ...
    if (center_vertex_degree >= petal_num) {
        ...
        for (uint32_t i = 0; i < petal_num; i++) {
            uint32_t rand_index = next_edge_distribution(this_rand_generator);
            while (vertex_set.find(rand_index) != vertex_set.end()) {
                rand_index = next_edge_distribution(this_rand_generator);
            }
    ...
    //  cout << center_vertex_id << " " << sampled_petals[0] 
...
*/
SamplingTreeNode *construct_sampling_tree(Graph &graph, Pattern &pattern, default_random_engine &this_rand_generator) {
    ...
    for (uint32_t i = 0; i < pattern.cycle_num; i++) {
        if (i == 0) {
            root = odd_cycle_sampler(graph, pattern.cycles[i].size(), this_rand_generator);
            ...
            for (uint32_t j = 0; j < leaves.size(); j++) {
                leaves[j]->children.push_back(odd_cycle_sampler(graph, pattern.cycles[i].size(), this_rand_generator));
            }
    ...
    for (uint32_t i = 0; i < pattern.star_num; i++) {
        if (root == nullptr) {
            root = star_sampler(graph, pattern.stars[i].size() - 1, this_rand_generator);
            for (uint32_t j = 0; j < root->children.size(); j++) {
                leaves.push_back(root->children[j]);
            }
        } else {
            // extend every leaf by a cycle tree
            for (uint32_t j = 0; j < leaves.size(); j++) {
                leaves[j]->children.push_back(star_sampler(graph, pattern.stars[i].size() - 1, this_rand_generator));
            }
...
void DFS_judge_remaining_edges(uint32_t idx, Graph &graph, Pattern &pattern,
    ...
    if (idx < cycle_vertexes.size()) {
        for (uint32_t i = 0; i < pattern.candidate_cycle_v_maps[idx].size(); i++) {
            ...
            // DFS_judge_further
            DFS_judge_remaining_edges(idx + 1, graph, pattern, cycle_vertexes, star_vertexes, v_map, found_instances);
...
uint32_t judge_remaining_edges(Graph &graph, Pattern &pattern,
    ...
    DFS_judge_remaining_edges(0, graph, pattern, cycle_vertexes, star_vertexes, v_map, found_instances);
...
void tree_DFS_helper(SamplingTreeNode *root, Graph &graph, Pattern &pattern,
    ...
    if (root->children.size() == 0) {
        ...
        if (test_succeed == false) {
            ...
            uint32_t num_matches = judge_remaining_edges(graph, pattern, cycle_vertexes, star_vertexes);
            if (num_matches == 0) {
                test_succeed = false;
            } else {
    ...
    for (uint32_t i = 0; i < root->children.size(); i++) {
        tree_DFS_helper(root->children[i], graph, pattern, path, v_map, total_prob_inverse, total_sampling_times);
    }
    path.pop_back();
...
*/
void sampling_tree_count(SamplingTreeNode *root, Graph &graph, Pattern &pattern, double &total_prob_inverse, uint64_t & total_sampling_times) {
    if (FLAG_print)
    ...
    tree_DFS_helper(root, graph, pattern, path, v_map, total_prob_inverse, total_sampling_times);
...
*/
tuple<double, uint64_t> estimate_pattern(Graph &graph, Pattern &pattern, uint64_t sampling_times, default_random_engine &this_rand_generator) {
    ...
    for (uint64_t i = 0; i < sampling_times; i++) {
        ...
        SamplingTreeNode *root = construct_sampling_tree(graph, pattern, this_rand_generator);
        ...
        sampling_tree_count(root, graph, pattern, this_prob_inverse, this_num_leaves);
