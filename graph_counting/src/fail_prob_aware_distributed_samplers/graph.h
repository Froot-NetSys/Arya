...
struct Block {
    Block(BlockType type_, uint32_t idx_, uint32_t fail_probability_ = 0, uint32_t total_sampled_times_ = 0) : type(type_), idx(idx_),
         fail_probability(fail_probability_), total_sampled_times(total_sampled_times_) {}

    BlockType type;
    uint32_t idx;
    uint32_t fail_probability;
    uint32_t total_sampled_times;

    friend bool operator < (Block x, Block y)
    {
        return x.fail_probability * y.total_sampled_times > y.fail_probability * x.total_sampled_times;
    }
};
    ...
    */
    void load() {
        ...
        for (uint32_t i = 0; i < cycle_num; i++) {
            ...
            blocks.push_back(Block(Cycle, i));
        }
